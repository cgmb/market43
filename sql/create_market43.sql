...
create table listing (ListingId int unsigned auto_increment,
  MinimumBid int unsigned,
  PostedTimestamp timestamp,
  ExpiryTimestamp timestamp,
  ListedItemId int unsigned,
  ListingUserId int unsigned,
  primary key (ListingId),
  foreign key (ListedItemId) references item(ItemId),
  foreign key (ListingUserId) references user(UserId));
create table transaction (TransactionId int unsigned,
  NetBalanceChange int,
  TransactionType tinyint,
  TransactionUser int unsigned, primary key (TransactionId),
